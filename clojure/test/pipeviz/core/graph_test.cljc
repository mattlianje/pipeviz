...
(deftest cron?-test
         (testing "valid cron expressions"
                  ...
                  (is (core/cron? "0 0 * * *"))
                  (is (core/cron? "*/15 * * * *"))
                  (is (core/cron? "0 9 * * MON-FRI")))
         (testing "invalid cron expressions"
                  (is (not (core/cron? nil)))
                  (is (not (core/cron? "")))
                  (is (not (core/cron? "daily")))
                  (is (not (core/cron? "0 0 * *")))
                  (is (not (core/cron? "0 0 * * * *")))))

(deftest format-schedule-test
         (testing "non-cron passthrough"
                  (is (= "daily" (core/format-schedule "daily")))
                  (is (nil? (core/format-schedule nil))))
         (testing "every N minutes"
                  (is (= "Every 15 minutes" (core/format-schedule "*/15 * * * *"))))
         (testing "every N hours"
                  (is (= "Every 2 hours" (core/format-schedule "0 */2 * * *"))))
         (testing "daily at specific time"
                  (is (= "Daily at 9 AM" (core/format-schedule "0 9 * * *")))
                  (is (= "Daily at 9:30 AM" (core/format-schedule "30 9 * * *"))))
         (testing "weekday patterns"
                  (is (= "Weekdays at 9 AM" (core/format-schedule "0 9 * * 1-5")))
                  (is (= "Mondays at 8 AM" (core/format-schedule "0 8 * * 1"))))
         (testing "day of month"
                  (is (= "1st of each month at 12 AM" (core/format-schedule "0 0 1 * *")))))
...
(deftest escape-dot-test
         (testing "escapes special characters"
                  (is (= "foo" (core/escape-dot "foo")))
                  (is (= "foo\\\"bar" (core/escape-dot "foo\"bar")))
                  (is (= "foo\\\\bar" (core/escape-dot "foo\\bar")))))

(deftest generate-dot-test
         (testing "generates valid DOT"
                  (let [dot (core/generate-dot test-config)]
         ...
         (testing "respects show-datasources? option"
                  (let [dot (core/generate-dot test-config {:show-datasources? false})]
                       (is (str/includes? dot "\"ingest\""))
                       (is (not (str/includes? dot "shape=ellipse"))))))

(deftest build-pipeline-adjacency-test
         (testing "builds correct adjacency maps"
                  (let [{:keys [upstream downstream]} (core/build-pipeline-adjacency test-config)]
...
(deftest reachable-test
         (testing "finds all reachable nodes"
                  (let [{:keys [downstream]} (core/build-pipeline-adjacency test-config)]
                       (is (= #{"transform" "load"} (core/reachable "ingest" #(get downstream % #{}))))
                       (is (= #{"load"} (core/reachable "transform" #(get downstream % #{}))))
                       (is (= #{} (core/reachable "load" #(get downstream % #{})))))))

(deftest upstream-downstream-test
         (testing "upstream-of"
                  (is (= #{"ingest" "transform"} (core/upstream-of test-config "load")))
                  (is (= #{"ingest"} (core/upstream-of test-config "transform")))
                  (is (= #{} (core/upstream-of test-config "ingest"))))

         (testing "downstream-of"
                  (is (= #{"transform" "load"} (core/downstream-of test-config "ingest")))
                  (is (= #{"load"} (core/downstream-of test-config "transform")))
                  (is (= #{} (core/downstream-of test-config "load")))))

(deftest lineage-with-depth-test
         (testing "returns nodes with depth"
                  (let [{:keys [downstream]} (core/build-pipeline-adjacency test-config)
                        lineage (core/lineage-with-depth downstream "ingest")]
                       (is (= 2 (count lineage)))
...
(deftest full-lineage-test
         (testing "returns complete lineage"
                  (let [{:keys [upstream downstream]} (core/full-lineage test-config "transform")]
                       (is (= 1 (count upstream)))
...
(deftest build-full-adjacency-test
         (testing "builds adjacency including datasources"
                  (let [{:keys [upstream downstream]} (core/build-full-adjacency test-config)]
...
(deftest full-graph-lineage-test
         (testing "lineage includes datasources"
                  (let [{:keys [upstream downstream]} (core/full-graph-lineage test-config "transform")]
      ...
      ;; Downstream of transform: clean, load
         ...
         (testing "lineage from datasource"
                  (let [{:keys [upstream downstream]} (core/full-graph-lineage test-config "raw")]
      ...
      ;; Downstream: transform, clean, load
         ...
         (testing "full chain from ingest"
                  (let [{:keys [downstream]} (core/full-graph-lineage test-config "ingest")]
...
(deftest validate-config-test
         (testing "valid config"
                  (let [{:keys [valid? errors]} (core/validate-config test-config)]
                       (is valid?)
                       (is (empty? errors))))

         (testing "nil config"
                  (let [{:keys [valid? errors]} (core/validate-config nil)]
                       (is (not valid?))
                       (is (some #(str/includes? % "nil") errors))))

         (testing "empty pipelines"
                  (let [{:keys [valid? errors]} (core/validate-config {:pipelines []})]
                       (is (not valid?))
                       (is (some #(str/includes? % "No pipelines") errors))))

         (testing "pipeline missing name"
                  (let [{:keys [valid? errors]} (core/validate-config {:pipelines [{:schedule "daily"}]})]
                       (is (not valid?))
                       (is (some #(str/includes? % "missing name") errors)))))

(deftest fuzzy-match-test
         (testing "exact substring match"
                  (let [{:keys [match score]} (core/fuzzy-match "transform" "trans")]
                       (is match)
                       (is (> score 0.5))))

         (testing "case insensitive"
                  (let [{:keys [match]} (core/fuzzy-match "Transform" "trans")]
                       (is match)))

         (testing "fuzzy character match"
                  (let [{:keys [match]} (core/fuzzy-match "user-enrichment" "ue")]
                       (is match)))

         (testing "no match"
                  (let [{:keys [match score]} (core/fuzzy-match "transform" "xyz")]
                       (is (not match))
                       (is (zero? score))))

         (testing "word boundary bonus"
                  (let [boundary-match (core/fuzzy-match "user_events" "ue")
                        no-boundary-match (core/fuzzy-match "superenrich" "ue")]
...
(deftest highlight-match-test
         (testing "substring match"
                  (is (= "trans<span class=\"result-match\">form</span>ation"
                         (core/highlight-match "transformation" "form"))))

         (testing "case insensitive highlight"
                  (is (= "<span class=\"result-match\">Trans</span>form"
                         (core/highlight-match "Transform" "trans"))))

         (testing "fuzzy match highlight"
                  (let [result (core/highlight-match "user" "ur")]
...
(deftest search-nodes-test
         ...
         (testing "results sorted by score"
                  (let [config {:pipelines [{:name "abc" :description "xyz"}
                                            {:name "abcdef"}]
                                :datasources []}
                        results (core/search-nodes config "abc")]
...
(deftest build-attribute-lineage-map-test
         (testing "builds attribute map"
                  (let [{:keys [attribute-map]} (core/build-attribute-lineage-map attr-test-config)]
      ...
      ;; target::source_id has upstream
         ...
         (testing "builds datasource map"
                  (let [{:keys [datasource-map]} (core/build-attribute-lineage-map attr-test-config)]
...
(deftest attribute-provenance-test
         (testing "returns provenance for attribute"
                  (let [{:keys [attribute-map]} (core/build-attribute-lineage-map attr-test-config)
                        prov (core/attribute-provenance attribute-map "target__source_id")]
         ...
         (testing "respects max-depth"
                  (let [{:keys [attribute-map]} (core/build-attribute-lineage-map attr-test-config)
                        prov-full (core/attribute-provenance attribute-map "target__source_id")
                        prov-depth1 (core/attribute-provenance attribute-map "target__source_id" {:max-depth 1})]
      ;; Should have same or fewer upstream with depth limit
         ...
         (testing "returns nil for unknown attribute"
                  (is (nil? (core/attribute-provenance {} "unknown__attr")))))
...
(deftest build-group-data-test
         (testing "builds group membership"
                  (let [{:keys [groups pipeline->group]} (core/build-group-data group-test-config)]
         ...
         (testing "computes group inputs/outputs"
                  (let [{:keys [groups]} (core/build-group-data group-test-config)
...
(deftest all-groups-test
         (testing "returns unique groups"
                  (is (= ["etl"] (core/all-groups group-test-config)))
                  (is (empty? (core/all-groups test-config)))))

(deftest generate-dot-with-collapsed-groups-test
         (testing "collapsed group renders as single node"
                  (let [dot (core/generate-dot group-test-config {:collapsed-groups #{"etl"}})]
      ...
      ;; Individual pipelines should not appear
         ...
         (testing "expanded group renders individual pipelines"
                  (let [dot (core/generate-dot group-test-config {:collapsed-groups #{}})]
...
(deftest kahns-waves-test
         (testing "simple linear chain"
                  (let [nodes #{"a" "b" "c"}
                        edges [["a" "b"] ["b" "c"]]
                        waves (core/kahns-waves nodes edges)]
                       (is (= [["a"] ["b"] ["c"]] waves))))
         ...
         (testing "returns nil for cyclic graph"
                  (let [nodes #{"a" "b" "c"}
                        edges [["a" "b"] ["b" "c"] ["c" "a"]]]
                       (is (nil? (core/kahns-waves nodes edges))))))

(deftest compute-execution-waves-test
         (testing "computes waves for selected pipelines"
                  (let [result (core/compute-execution-waves (:pipelines test-config) ["ingest"])]
                       (is (some? result))
         ...
         (testing "returns nil for empty selection"
                  (is (nil? (core/compute-execution-waves (:pipelines test-config) [])))))

;; Blast radius tests
(deftest build-blast-radius-graph-test
         (testing "builds graph structure"
                  (let [g (core/build-blast-radius-graph test-config)]
                       (is (map? (:downstream g)))
                       (is (map? (:node-types g)))
...
(deftest blast-radius-for-node-test
         (testing "computes blast radius from pipeline"
                  (let [g (core/build-blast-radius-graph test-config)
                        result (core/blast-radius-for-node g "ingest")]
         ...
         (testing "blast from datasource"
                  (let [g (core/build-blast-radius-graph test-config)
                        result (core/blast-radius-for-node g "raw")]
         ...
         (testing "returns nil for nil node"
                  (let [g (core/build-blast-radius-graph test-config)]
                       (is (nil? (core/blast-radius-for-node g nil))))))

;; Backfill analysis tests
(deftest generate-backfill-analysis-test
         (testing "generates backfill plan"
                  (let [result (core/generate-backfill-analysis test-config ["ingest"])]
         ...
         (testing "returns nil for empty selection"
                  (is (nil? (core/generate-backfill-analysis test-config []))))

         (testing "throws for non-pipeline nodes"
                  (is (thrown? #?(:clj Exception :cljs js/Error)
                               (core/generate-backfill-analysis test-config ["raw"])))))

;; =============================================================================
;; Edge deduplication tests
;; =============================================================================

(deftest generate-dot-edge-dedup-test
         (testing "edges are deduplicated"
                  (let [config {:pipelines [{:name "p1" :input_sources ["ds1"] :output_sources ["ds2"]}
                                            {:name "p2" :input_sources ["ds1"] :output_sources ["ds2"]}]
                                :datasources [{:name "ds1"} {:name "ds2"}]}
                        dot (core/generate-dot config)]
      ;; ds1 -> p1 and ds1 -> p2 should both appear (different targets)
                       (is (str/includes? dot "\"ds1\" -> \"p1\""))
                       (is (str/includes? dot "\"ds1\" -> \"p2\""))
      ;; Each edge should appear only once
                       (is (= 1 (count (re-seq #"\"ds1\" -> \"p1\"" dot))))
                       (is (= 1 (count (re-seq #"\"ds1\" -> \"p2\"" dot)))))))

;; =============================================================================
;; Blast radius with groups tests
;; =============================================================================

(deftest blast-radius-for-group-test
         (testing "computes blast radius for group"
                  (let [g (core/build-blast-radius-graph group-test-config)
                        result (core/blast-radius-for-node g "etl")]
                       (is (= "etl" (:source result)))
                       (is (= :group (:source-type result)))
                       (is (= #{"etl-1" "etl-2"} (set (:group-members result))))
      ;; Downstream should include report (which reads from clean/enriched)
                       (is (some #(= "report" (:name %)) (:downstream result)))))

         (testing "group blast radius has correct member count"
                  (let [g (core/build-blast-radius-graph group-test-config)
                        result (core/blast-radius-for-node g "etl")]
                       (is (= 2 (:group-size result))))))
