                         ...
                         (when show-datasources?
                           (concat
                            (for [p pipelines :let [tgt (effective-node (:name p))]
                                  src (:input_sources p) :let [k [src tgt]] :when (not (@seen k)) :let [_ (swap! seen conj k)]]
                              (str "\"" (escape-dot src) "\" -> \"" (escape-dot tgt) "\" [color=\"" edge-clr "\" arrowsize=0.8];"))
                            (for [p pipelines :let [src (effective-node (:name p))]
                                  out (:output_sources p) :let [k [src out]] :when (not (@seen k)) :let [_ (swap! seen conj k)]]
                              (str "\"" (escape-dot src) "\" -> \"" (escape-dot out) "\" [color=\"" edge-clr "\" arrowsize=0.8];"))))
                         (let [valid (set (map :name pipelines))]
                           (for [p pipelines, up (:upstream_pipelines p) :when (valid up)
                                 :let [s (effective-node up), t (effective-node (:name p)), k [s t]]
                                 :when (and (not= s t) (not (@seen k))) :let [_ (swap! seen conj k)]]
                             (str "\"" (escape-dot s) "\" -> \"" (escape-dot t) "\" [color=\""
                                  styles/dependency-edge-color "\" style=solid arrowsize=0.8];")))
                         (when (seq collapsed-groups)
                           (for [g collapsed-groups :let [{:keys [outputs]} (groups g)]
                                 out outputs, p pipelines
                                 :when (and (some #{out} (:input_sources p)) (not (collapsed-groups (:group p))))
                                 :let [s (str "group:" g), t (:name p), k [s t]]
                                 :when (not (@seen k)) :let [_ (swap! seen conj k)]]
                             (str "\"" (escape-dot s) "\" -> \"" (escape-dot t) "\" [color=\"" edge-clr "\" arrowsize=0.8];"))))))]
        ...
        collect-edges (fn ce [attrs ds-id prefix]
                        (mapcat (fn [a]
                                  (let [path (if (seq prefix) (str prefix "__" (:name a)) (:name a))
                                    ...
                                    (concat
                                     (for [src froms :let [{:keys [id]} (parse-source-ref src)
                                                           as (cond-> [(str "color=\"" (:edge styles/attr-graph) "\"")]
                                                                (@structs id) (conj (str "ltail=\"cluster_" id "\""))
                                                                (@structs tid) (conj (str "lhead=\"cluster_" tid "\"")))]] (str "  \"" id "\" -> \"" tid "\" [" (str/join " " as) "];"))
...
(defn build-blast-radius-graph [config]
  (when config
    (let [pipelines (or (:pipelines config) [])
          downstream (atom {}), node-types (atom {})]
      (doseq [p pipelines]
        (swap! node-types assoc (:name p) :pipeline)
        (swap! downstream update (:name p) #(or % #{}))
        (doseq [s (:output_sources p)]
          (swap! downstream update (:name p) conj s)
          (swap! node-types update s #(or % :datasource)))
        (doseq [s (:input_sources p)]
          (swap! downstream update s #(or % #{}))
          (swap! downstream update s conj (:name p))
          (swap! node-types update s #(or % :datasource)))
        (doseq [u (:upstream_pipelines p)]
          (swap! downstream update u #(or % #{}))
          (swap! downstream update u conj (:name p))))
      (doseq [ds (:datasources config)]
        (swap! node-types update (:name ds) #(or % :datasource)))
      {:downstream @downstream :node-types @node-types :pipelines pipelines
...
(defn blast-radius-for-node [{:keys [downstream node-types pipelines groups]} node-name]
  (when node-name
    (let [members (get groups node-name), is-group? (seq members)
          starts (if is-group? members [node-name]), mset (set starts)
          visited (atom (into {} (map #(vector % 0) starts)))
          edges (atom []), queue (atom (vec (map #(vector % 0) starts)))]
      (while (seq @queue)
        (let [[curr depth] (first @queue)]
          (swap! queue #(vec (rest %)))
          (doseq [nb (get downstream curr #{})]
            (when-not (and is-group? (mset curr) (mset nb))
              (swap! edges conj {:source (if is-group? node-name curr) :target nb})
              (when-not (@visited nb)
                (swap! visited assoc nb (inc depth))
                (swap! queue conj [nb (inc depth)]))))))
      (let [nodes (->> @visited
                       ...
                       (sort-by (juxt :depth :name)) vec)]
        (cond-> {:source node-name :source-type (if is-group? :group (node-types node-name :unknown))
                 :total-affected (count nodes) :max-depth (if (seq nodes) (apply max (map :depth nodes)) 0)
                 :downstream nodes :by-depth (into (sorted-map) (group-by :depth nodes))
