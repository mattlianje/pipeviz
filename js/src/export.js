...
let plannerGraphInstance = null
let selectedPlannerPipelines = []
...
export function generatePlannerAnalysis(nodeNames) {
    if (!state.currentConfig) return null
    ...
    if (invalidNodes.length > 0) {
        return {
            nodes: sourceNodes,
            message: `Planner is only available for pipelines. Invalid: ${invalidNodes.join(', ')}`,
            waves: []
        }
    }
    ...
    if (allDownstream.length === 0 && trueSources.length === sourceNodes.length) {
        return {
            nodes: sourceNodes,
            message: 'No downstream pipelines to plan.',
            waves: []
        }
    }
...
export function populatePlannerSelect() {
    const list = document.getElementById('planner-picker-list')
    if (!list) return

    list.innerHTML = ''
    selectedPlannerPipelines = []
    updatePickerButton()
    updatePickerCount()

    const filterInput = document.getElementById('planner-picker-filter')
    if (filterInput) filterInput.value = ''

    if (filterInput) {
        filterInput.placeholder = currentPlannerView === 'blast' ? 'Filter nodes...' : 'Filter pipelines...'
    }
    ...
    sortedGroups.forEach((groupName) => {
        ...
        groupLabel.className = 'planner-picker-group'
        ...
        grouped[groupName].forEach((item) => {
            ...
            itemEl.className = 'planner-picker-item'
...
function handlePipelineCheckChange(e) {
    ...
    if (isBlastMode) {
        selectedPlannerPipelines = [value]
    } else {
        if (e.target.checked) {
            if (!selectedPlannerPipelines.includes(value)) {
                selectedPlannerPipelines.push(value)
            }
        } else {
            selectedPlannerPipelines = selectedPlannerPipelines.filter((p) => p !== value)
        }
    }
    updatePickerButton()
    updatePickerCount()
    ...
    updateHashWithPlannerState(currentPlannerView, selectedPlannerPipelines)
}

export function filterPlannerItems(query) {
    const list = document.getElementById('planner-picker-list')
    if (!list) return
    ...
    list.querySelectorAll('.planner-picker-item').forEach((item) => {
    ...
    list.querySelectorAll('.planner-picker-group').forEach((label) => {
        label.classList.toggle('hidden', !visibleGroups.has(label.dataset.group))
    })
}

export function togglePlannerPicker() {
    const dropdown = document.getElementById('planner-picker-dropdown')
    if (dropdown) {
...
function closePickerOnClickOutside(e) {
    const picker = document.querySelector('.planner-pipeline-picker')
    const viewBtns = document.querySelectorAll('.planner-view-btn')
...
export function clearPlannerSelection() {
    ...
    updateHashWithPlannerState(currentPlannerView, [])
}

function updatePickerButton() {
    const countEl = document.getElementById('planner-selected-count')
    if (countEl) {
        const count = selectedPlannerPipelines.length
...
export function setPlannerView(view, skipHashUpdate = false) {
    ...
    plannerGraphInstance = null
    const graphContainer = document.getElementById('planner-graph')
    if (graphContainer && graphContainer.parentNode) {
        const parent = graphContainer.parentNode
        const newContainer = document.createElement('div')
        newContainer.id = 'planner-graph'
        newContainer.className = 'planner-graph'
    ...
    if (!skipHashUpdate) {
        updateHashWithPlannerState(view, selectedPlannerPipelines)
    }
}
...
export function updatePlannerPlan() {
    const output = document.getElementById('planner-output')
    const graphContainer = document.getElementById('planner-graph')
    if (!output) return

    if (selectedPlannerPipelines.length === 0) {
        output.textContent = 'Select pipelines to see the planner plan...'
        if (graphContainer) {
    ...
    const pipelineAnalysis = generatePlannerAnalysis(selectedPlannerPipelines)

    if (!pipelineAnalysis) {
        output.textContent = 'Unable to generate planner plan. Make sure a configuration is loaded.'
        if (graphContainer) {
            graphContainer.innerHTML =
    ...
    if (currentPlannerView === 'airflow') {
        ...
        if (graphContainer && airflowAnalysis.waves && airflowAnalysis.waves.length > 0) {
            const dot = generateAirflowPlannerDot(airflowAnalysis)
            if (dot) {
                try {
                    if (!plannerGraphInstance) {
                        graphContainer.innerHTML = ''
                        plannerGraphInstance = d3
                            .select('#planner-graph')
                            .graphviz()
                            .width(graphContainer.clientWidth)
                            .height(graphContainer.clientHeight || 450)
                            .fit(true)
                            .zoom(true)
                            .transition(function () {
                    ...
                    plannerGraphInstance.renderDot(dot)
                } catch (e) {
                    graphContainer.innerHTML =
    ...
    if (currentPlannerView === 'blast') {
        const sourceNode = selectedPlannerPipelines[0]
    ...
    if (graphContainer && pipelineAnalysis.waves && pipelineAnalysis.waves.length > 0) {
        const dot = generatePlannerDot(pipelineAnalysis)
        if (dot) {
            try {
                if (!plannerGraphInstance) {
                    graphContainer.innerHTML = ''
...
export function copyPlannerToClipboard(event) {
    const output = document.getElementById('planner-output')
    if (!output) return
    ...
    if (pipelines && pipelines.length > 0) {
        ...
        pipelines.forEach((pipelineName) => {
            ...
            if (input) {
                input.checked = true
                if (isBlastMode) {
                    selectedPlannerPipelines = [pipelineName]
                } else {
                    if (!selectedPlannerPipelines.includes(pipelineName)) {
                        selectedPlannerPipelines.push(pipelineName)
                    }
                }
            }
        })
