...
export function renderAttributeGraph() {
    ...
    if (!state.attributeGraphviz) {
        const el = document.getElementById('attribute-graph')
        el.innerHTML = ''
        const w = el.clientWidth || el.parentElement.clientWidth || 800
        const h = el.clientHeight || el.parentElement.clientHeight || 600
        state.attributeGraphviz = d3.select('#attribute-graph').graphviz().width(w).height(h).fit(true).zoom(true)
    }

    state.attributeGraphviz.renderDot(dot).on('end', () => {
        ...
        d3.select('#attribute-graph svg').on('click', (event) => {
...
export function buildAttributeLineageMap() {
    ...
    const dsDirectUpstream = {}
    const dsDirectDownstream = {}

    datasources.forEach((ds) => {
        const id = ds.name.replace(/[^a-zA-Z0-9]/g, '_')
    ...
    Object.values(state.attributeLineageMap).forEach((attr) => {
        ...
        const dsId = attr.datasource.replace(/[^a-zA-Z0-9]/g, '_')

        const upIds = attr.upstream || []
        for (let i = 0; i < upIds.length; i++) {
            const upDs = upIds[i].split('__')[0]
            if (upDs && upDs !== dsId && dsDirectUpstream[dsId]) {
                dsDirectUpstream[dsId].add(upDs)
            }
        }

        const downIds = attr.downstream || []
        for (let j = 0; j < downIds.length; j++) {
            const downDs = downIds[j].split('__')[0]
    ...
    function bfsProvenance(startId, getNeighbors) {
        ...
        const queue = []
        const neighbors = getNeighbors(startId)
        ...
        while (queue.length > 0) {
            ...
            result.push(item)
            const next = getNeighbors(item.id)
            next.forEach((nextId) => {
    ...
    datasources.forEach((ds) => {
        ...
        state.datasourceLineageMap[id] = {
            name: ds.name,
            upstream: bfsProvenance(id, (x) => dsDirectUpstream[x] || new Set()),
            downstream: bfsProvenance(id, (x) => dsDirectDownstream[x] || new Set())
        }
    })
}
...
export function showAttributeDetails(attrId, upstream, downstream) {
    ...
    if (sortedUpstream.length === 0 && sortedDownstream.length === 0) {
        html += `<div class="detail-value" style="color: var(--text-muted);">No lineage connections</div>`
    }
...
export function showDatasourceInAttributePanel(ds) {
    ...
    const dsId = ds.name.replace(/[^a-zA-Z0-9]/g, '_')
    const dsLineage = state.datasourceLineageMap[dsId] || { upstream: [], downstream: [] }
    const upstreamList = dsLineage.upstream || []
    const downstreamList = dsLineage.downstream || []

    const connectedIds = new Set()
    upstreamList.forEach((x) => {
        connectedIds.add(x.id)
    })
    downstreamList.forEach((x) => {
        connectedIds.add(x.id)
    })
    ...
    if (upstreamList.length > 0) {
        ...
        html += '<div class="detail-value">'
        upstreamList.forEach((item) => {
            const depth = item.depth
            const indent = (depth - 1) * 16
            const opacity = Math.max(0.4, 1 - (depth - 1) * 0.2)
            const found = (state.currentConfig.datasources || []).find(
                (d) => d.name.replace(/[^a-zA-Z0-9]/g, '_') === item.id
            )
            const name = found ? found.name : item.id
